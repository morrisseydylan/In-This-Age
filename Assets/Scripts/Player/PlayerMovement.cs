...
public class PlayerMovement : MonoBehaviour
{
    ...
    {
        if (Input.GetKeyDown(KeyCode.Mouse0) && !disabled)
        {
            PlayerAnimator.SetBool("isAttacking", true);
        }

        if (Input.GetKeyUp(KeyCode.Mouse0) && !disabled)
        {
    ...
    void HandleMovement(float HorizontalMovement, float VerticleMovement)
    {
        ...
        {
            float value = (PlayerAnimator.GetBool("isAttacking")) ? 0.25f : 1;
            rb.velocity = new Vector2(HorizontalMovement * PlayerMovementSpeed * value, rb.velocity.y);
        }
    ...
    {
        float value = (Input.mousePosition.x > Screen.width / 2) ? 1 : -1; //(HorizontalVelocity == 0) ? lastVelocity : HorizontalVelocity;
